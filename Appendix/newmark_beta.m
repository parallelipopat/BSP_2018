function [x_new, v_new] = newmark_beta(beta, gamma, x_i, v_i, h, ... 
                                       update_acceleration, d_acceleration)
    a_i = update_acceleration(x_i);
    y_i = [v_i; x_i];
  
    % Newton-Raphson iteration
    f = @(z, v_i, a_i, h) [(1-gamma)*a_i + gamma*update_acceleration(z); ...
                  v_i + 0.5*h*((1-2*beta)*a_i + 2*beta*update_acceleration(z))];
