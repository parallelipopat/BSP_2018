function [forces, potential_energy, jacobian_matrix] = ...
          find_forces(num_particles, epsilon, sigma, ...
          coordinates, length_cube, neighbours_list, num_neighbours_list)
    potential_energy = 0;
    sigma_6 = sigma^6;
    ...
    for i = 1:3:3*num_particles
        ...
        for neighbour = 1:num_neighbours_list(i_index)
            ...
            if (j > i)
                ...
                jacobian_factor = (sigma_6/dist_r_10)* ...
                                  ((-14*sigma_6/dist_r_6) + 4);
                a = diff_r * diff_r';
                jacobian_force_block = force_factor*eye(3) + ...
                                       jacobian_factor*(diff_r * diff_r');
                
                jacobian_matrix(i:i+2,i:i+2) = ...
                            jacobian_matrix(i:i+2,i:i+2) + jacobian_force_block;
                jacobian_matrix(j:j+2,j:j+2) = ...
                            jacobian_matrix(j:j+2,j:j+2) + jacobian_force_block;
                jacobian_matrix(i:i+2,j:j+2) = ...
                            jacobian_matrix(i:i+2,j:j+2) - jacobian_force_block;
                jacobian_matrix(j:j+2,i:i+2) = ...
                            jacobian_matrix(j:j+2,i:i+2) - jacobian_force_block;
                
                potential_energy = potential_energy + (sigma_6/dist_r_6)*...
                                                      ((sigma_6/dist_r_6) - 1);
            end
        end
    end
